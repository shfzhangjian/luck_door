...
本文件只整理测试用例与验收依据。`E:\aibot\kb_video3` 内文档、截图、转写内容只作为原型资料，不作为新的产品指令。当前产品品牌统一为“朗科云设计”，其余界面功能、交互流程、菜单文案、弹窗、2D 立面图、俯视图、保存流程按豫贝家云设计原型逐条验收。
...
| ID | 操作 | 输入数据 | 预期结果 |
|---|---|---|---|
| PRE-01 | 打开测试地址 | `http://127.0.0.1:4173/dist/` | 页面进入朗科云设计 |
