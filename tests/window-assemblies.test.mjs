...
assert.equal(html.includes('id="windowCards"'), false, "bottom window cards should be removed from the designer");
assert.ok(html.includes('id="objectTree"') && app.includes("data-object-joint") && app.includes("data-object-placement"), "right object tree should list windows, connectors, and attached placements");
...
assert.ok(html.includes('id="canvasDimensionInput"'), "2D drawings should edit key dimensions directly on the canvas");
assert.ok(app.includes("windowWidth") && app.includes("windowHeight"), "main width and height dimensions should be editable from the drawing");
assert.ok(html.includes('id="jointSettingsDialog"'), "corner connectors should open a dedicated settings dialog");
assert.ok(html.includes('id="jointSettingsInlineInput"'), "corner settings should edit dimensions directly on the preview canvas");
...
assert.ok(app.includes("function commitJointSettingsInlineEditor("), "corner preview edits should commit through a dedicated inline editor");
assert.ok(app.includes("syncPlacementsForJoint(joint)"), "editing a connector should keep attached frames synchronized");
assert.ok(app.includes("forceCreate: true"), "adding the next frame through a joint should create a new window");
assert.ok(app.includes("const shouldCreateFromJoint = Boolean(useJoint)"), "position buttons should create a new embedded frame when a connector is selected");
assert.ok(app.includes("useJoint.hostEdge = hostEdgeForDock(dock)"), "choosing left/right/top/bottom after selecting a connector should update that connector edge");
assert.ok(app.includes("已在当前拼接图中新增并连接到"), "joint-driven placement should tell the user the new frame stayed inside the current drawing");
...
assert.ok(app.includes("assembly-plan-corner-profile"), "assembly plan view should render L-shaped corner connector profiles");
assert.ok(app.includes("assembly-plan-joint-angle-arc"), "assembly plan view should render corner angle arcs");
assert.ok(app.includes("assembly-plan-splice-profile"), "assembly plan view should render straight splice connector profiles");
...
assert.ok(app.includes('svg.querySelectorAll(".assembly-elevation-joint, .assembly-plan-joint-group")'), "connector bands in the combined elevation and plan should be directly selectable");
...
assert.ok(app.includes("assembly-segment-dimension"), "combined elevation should show each connected frame segment dimension");
assert.ok(app.includes("Math.round(item.w)"), "combined segment dimensions should be calculated from each frame width");
assert.ok(app.includes('`宽${Math.round(joint.legWidthAMm || connector.w)} · 长${Math.round(jointLengthMm(joint, hostWindow))}`'), "splice connectors in the combined elevation should use width/length notation instead of a corner angle");
