...
assert.ok(app.includes("function offsetPolygonPoints("), "DIY polygon frames should generate an inner frame path from the actual polygon edges");
...
assert.ok(app.includes("function saveDiyShapeElement("), "saving the DIY canvas should store a named reusable window-frame shape");
...
assert.ok(app.includes("function openCustomShapePlacementDialog("), "saved DIY window frames should ask only for a new placement around the current window");
assert.ok(app.includes("function createWindowFromCustomShapeElement("), "saved DIY window frames should create a new window instead of replacing a selected cell");
assert.ok(app.includes("function editCustomShapeElement("), "saved DIY outlines should be maintainable from the drawing tool library");
...
assert.ok(app.includes("function updateCellsUsingCustomShape("), "legacy cell-level DIY usages should remain updatable when old projects are opened");
assert.ok(app.includes("function normalizeCellCustomShape("), "legacy DIY cell geometry should continue to normalize for backward compatibility");
assert.ok(app.includes("function cellCustomShapePath("), "legacy 2D elevations should still render imported cell-level DIY outlines");
assert.ok(app.includes("function addThreeCustomCellGeometry("), "legacy 3D preview should still build geometry from imported DIY cell outlines");
assert.ok(app.includes("function addThreeShapeGlassPane("), "3D clipped fixed glass should not add a frame that does not exist in the 2D drawing");
