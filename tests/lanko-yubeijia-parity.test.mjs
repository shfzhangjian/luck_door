...
assert.ok(app.includes('mode: "add_cell_markup"'), "TC-14 clicking a markup tool should enter cell markup placement mode");
assert.ok(app.includes("createCellMarkup(markupType, {") && app.includes('hostType: "cell"') && app.includes("hostCellId: cell.cellId"), "TC-14 placing a markup should attach it to the concrete target cell instead of creating a new window");
...
assert.ok(app.includes("cell.markups.push(markup)"), "TC-14 markups and holes should be stored on the existing cell");
...
assert.ok(app.includes("group.setAttribute(\"transform\"") && app.includes('const margin = found.markup.kind === "text" ? 0 : 4') && app.includes("found.markup.xPercent = Math.max(margin"), "TC-14 dragging a markup should preview the move, persist the cell-relative position, and keep holes or hardware inside their host cell");
...
assert.ok(app.includes("...project.windows.filter(win => win.windowId !== selected?.windowId)"), "TC-14 selecting an existing markup should search all current-canvas windows");
assert.ok(app.includes("openCanvasMarkupEditor(markup.markupId, event)") && app.includes("请输入文字后回车保存"), "TC-14 text placement should immediately open the text input after dropping on glass");
...
assert.ok(app.includes('mode: "add_root_window"') && app.includes("function createRootWindowFromCanvasCommand("), "Empty-canvas tool clicks should enter a visible placement mode and create the first window only after clicking the canvas");
assert.ok(app.includes("rootWindowPlacementZone") && app.includes("点击放置"), "Empty canvas should render a clickable first-window placement zone");
...
assert.ok(app.includes('const placement = createAssemblyPlacement(win.windowId, referenceWindow.windowId, "right"'), "TC-05 additional frame tools should attach the new frame to the current canvas instead of creating an invisible standalone window");
assert.ok(app.includes('showToast(`已生成${shapeLabel(type)}窗框。`)'), "TC-05 frame creation should confirm a generated frame");
