...
assert.ok(app.includes("function renderMarkupLayer(") && app.includes('const id = options.id || "markupLayer"'), "TC-14 elevation should render cell markups and glass holes on a dedicated markup layer");
assert.ok(app.includes("function renderWindowRootMarkups") && app.includes("root-markup-layer text-markup-layer"), "TC-14 elevation should render root text annotations above the whole window");
assert.ok(app.includes('id: "textMarkupLayer"') && app.includes("assemblyTextMarkupParts") && app.includes("text-markup-layer"), "TC-14 text annotations should be rendered on a final top layer after dimensions and canvas helpers");
assert.ok(app.includes("function normalizeMarkupTextSize") && app.includes('data-markup-field="sizeMm"') && app.includes("parseMarkupTextEditor"), "TC-14 text annotations should expose adjustable size in addition to 360 degree flat rotation");
assert.ok(app.includes("function normalizeMarkupTextDirection") && app.includes('data-markup-field="direction"') && app.includes('class="cell-markup-text vertical"'), "TC-14 text annotations should support horizontal or vertical font direction independently from rotation");
assert.ok(app.includes('class="cell-markup-rotate-handle"') && app.includes("function beginMarkupRotation") && css.includes(".cell-markup-rotate-handle"), "TC-14 text annotations should expose an on-canvas rotate handle instead of requiring typed angles only");
assert.ok(app.includes('class="cell-markup-resize-handle"') && app.includes("function beginMarkupResize") && css.includes(".cell-markup-resize-handle"), "TC-14 text annotations should expose an on-canvas resize handle instead of requiring typed size only");
assert.ok(app.includes("if (win.layout.columns.length > 1)") && app.includes("if (win.layout.rows.length > 1)"), "TC-14 assembly dimensions should avoid duplicate single-row or single-column labels that visually overlap");
